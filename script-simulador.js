...
// Version 7 - Fully resilient with safe translations, elements guards, and safe localStorage.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Se não estivermos na página do simulador, encerra a execução do simulador com segurança
    if (!countrySelect) {
        return;
    }
    ...
    async function handleCountryChange() {
        ...
        if (!countryCode) {
            if (operatorSelect) {
                operatorSelect.innerHTML = `<option value="">${t('select_country_first')}</option>`;
